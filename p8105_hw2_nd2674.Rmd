...
The dataset contains information about various trains in NYC and the stations each train transits through. Variables of importance include, stations, geographical coordinates of stations, route number and names along with entrance information. So far, the dataset has been filtered to include 19 variables of importance and entry has been changed into a logical variable. There are `r nrow(transit_df)` rows x `r ncol(transit_df)` columns in the dataset. The data is not tidy as it contains a set of variables namely `route1` to `route11` that are in wide format and can be condensed to long format.

* The total number of distinct stations identified both by name and by line are  `r transit_df %>% distinct(line, station_name) %>% count()` 
* The total stations that are ADA compliant are `r filter(transit_df, ada == TRUE) %>% distinct(line, station_name) %>% count() `
* Stations entrances that exist without vending allow entrance are : `r (filter(transit_df, vending == 'NO') %>% distinct(line, station_name, entrance_type) %>% count()) / (transit_df %>% distinct(line, station_name) %>% count()) `
...
* The total number of distinct stations identified both by name and by line serving A train are  `r filter(transit_tidy_df, route_number == "A")  %>% distinct(line, station_name) %>% count()` 
* The total stations that are ADA compliant of the ones serving the A train are `r filter(transit_tidy_df, route_number == "A" & ada == "TRUE")  %>% distinct(line, station_name) %>% count()` 
